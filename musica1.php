...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (!empty($titulo) && !empty($artista) && !empty($ano) && !empty($genero) && !empty($imagem['name']) && !empty($arquivo['name']) && !empty($descricao)) {
        ...
        if (move_uploaded_file($imagem['tmp_name'], $imagemNome) && move_uploaded_file($arquivo['tmp_name'], $arquivoNome)) {
            ...
            $stmt = $pdo->prepare("INSERT INTO musica1 (titulo, artista, ano, genero, arquivo, imagem) VALUES (?, ?, ?, ?, ?, ?)");
            if ($stmt->execute([$titulo, $artista, $ano, $genero, $arquivoNome, file_get_contents($imagemNome)])) {
...
<body>
    <div class="container">
        ...
        <h2>Criar Música</h2>
        <form method="post" action="Musica1.php" class="music-form" enctype="multipart/form-data">

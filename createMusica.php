...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (!empty($titulo) && !empty($artista) && !empty($ano) && !empty($genero) && !empty($imagem['name']) && !empty($arquivo['name'])) {
        ...
        if (move_uploaded_file($imagem['tmp_name'], $imagemNome) && move_uploaded_file($arquivo['tmp_name'], $arquivoNome)) {
            ...
            $stmt = $pdo->prepare("INSERT INTO musicas (titulo, artista, ano, genero, imagem, arquivo) VALUES (?, ?, ?, ?, ?, ?)");
            if ($stmt->execute([$titulo, $artista, $ano, $genero, $imagemNome, $arquivoNome])) {
...
<body>
    <div class="container">
        ...
        <h2>Criar Música</h2>
        <form method="post" action="createMusica.php" class="music-form" enctype="multipart/form-data">

...
if (isset($_GET['id'])) {
    ...
    $stmt = $pdo->prepare("SELECT * FROM musica1 WHERE id = ?");
    $stmt->execute([$id]);
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (!empty($titulo) && !empty($artista) && !empty($ano) && !empty($genero)) {
        ...
        $stmt = $pdo->prepare("UPDATE musica1 SET titulo = ?, artista = ?, ano = ?, genero = ?, arquivo = ?, imagem = ? WHERE id = ?");
        if ($stmt->execute([$titulo, $artista, $ano, $genero, $arquivoCaminho, $imagemConteudo, $id])) {
...
<body>
    <div class="container">
        <a href="musica1.php" class="back-button">Voltar</a>
        <h2>Atualizar Música</h2>
        <form method="post" action="updatemusica1.php?id=<?= $id ?>" class="music-form" enctype="multipart/form-data">
            ...
            <textarea name="descricao" id="descricao" required><?= htmlspecialchars($musica['descricao']) ?></textarea><br>
